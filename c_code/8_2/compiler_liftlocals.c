		...
		i_2 = ((NI) 1);
		{
			while (1) {
				...
				i = i_2;
				{
					NIM_BOOL T10_;
					T10_ = (NIM_BOOL)0;
					T10_ = ((*(*params)._Ukind.S6.sons->data[i]).kind == ((tyEnum_TNodeKind_G4E4Gxe7oI2Cm03rkiOzQw) 3));
					if (!(T10_)) goto LA11_;
					T10_ = ((*(*(*(*params)._Ukind.S6.sons->data[i])._Ukind.S4.sym).name).Sup.id == (*(*dest)._Ukind.S5.ident).Sup.id);
					LA11_: ;
					if (!T10_) goto LA12_;
					result = (*(*params)._Ukind.S6.sons->data[i])._Ukind.S4.sym;
					goto BeforeRet_;
				}
				LA12_: ;
				i_2 += ((NI) 1);
	...
	{
		{
			NIM_BOOL T4_;
			T4_ = (NIM_BOOL)0;
			T4_ = interestingVar_NYtvGlXG4WF2jKlY2I639awliftlocals((*it)._Ukind.S4.sym);
			if (!T4_) goto LA5_;
			asgnRefNoCycle((void**) (&(*n)._Ukind.S6.sons->data[i]), lookupOrAdd_6aiVjS5FtaP6i63MT9buQJw(c, (*it)._Ukind.S4.sym, (*it).info));
		}
		LA5_: ;
	}
	break;
