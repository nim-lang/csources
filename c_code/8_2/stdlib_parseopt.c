...
static NIM_CONST TY194407 TMP1582 = {
...
N_NIMCALL(void, handleshortoption_191291)(optparser191011* p) {
	...
	}
	{
		...
		i = parseword_191204((*p).Cmd, i, (&(*p).Val), TMP1582);
	}
	LA5: ;
	{
...
{	i = (*p).Pos;
	...
	case 45:
	{
		i += ((NI) 1);
		{
			...
			(*p).Kind = ((NU8) 2);
			i += ((NI) 1);
			i = parseword_191204((*p).Cmd, i, (&(*p).Key), TMP1583);
			{
				while (1) {
			...
			}
			{
				...
				(*p).Pos = parseword_191204((*p).Cmd, i, (&(*p).Val), TMP1582);
			}
			goto LA15;
			LA17: ;
			{
	...
	{
		(*p).Kind = ((NU8) 1);
		(*p).Pos = parseword_191204((*p).Cmd, i, (&(*p).Key), TMP1582);
	}
	break;
	}
	}BeforeRet: ;
...
TMP706[TMP1578+5].kind = 1;
TMP706[TMP1578+5].offset = TMP1578;
TMP706[TMP1578+5].name = TMP1577[TMP1578];
TMP1576[TMP1578] = &TMP706[TMP1578+5];
}
TMP706[9].len = 4; TMP706[9].kind = 2; TMP706[9].sons = &TMP1576[0];
...
TMP706[11].name = "val";
TMP706[0].len = 6; TMP706[0].kind = 2; TMP706[0].sons = &TMP1575[0];
