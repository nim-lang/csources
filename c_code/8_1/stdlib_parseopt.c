...
STRING_LITERAL(TMP1664, "", 0);
static NIM_CONST TY202404 TMP1665 = {
...
static NIM_CONST TY202404 TMP1668 = {
0x01, 0x02, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
;
static NIM_CONST TY202404 TMP1673 = {
...
N_NIMCALL(NimStringDesc*, quote_199287)(NimStringDesc* s) {
	...
	result = 0;
	{
		...
		LOC3 = 0;
		LOC4 = 0;
		LOC4 = nsuFindCharSet(s, TMP1665, ((NI) 0));
		LOC3 = (((NI) 0) <= LOC4);
		if (!(LOC3)) goto LA5;
		...
		if (!LOC3) goto LA6;
		{
			...
			i = parseword_199218(s, ((NI) 0), (&result), TMP1666);
			{
...
N_NIMCALL(void, initoptparser_199325)(NimStringDesc* cmdline, Optparser199205* Result) {
	...
	LA3: ;
	{
		unsureAsgnRef((void**) (&(*Result).cmd), copyString(((NimStringDesc*) &TMP1664)));
		{
			NI i_199340;
			NI HEX3Atmp_199343;
			NI res_199346;
...
appendString((*Result).cmd, LOC10);
	...
	LA1: ;
	(*Result).kind = ((NU8) 0);
	unsureAsgnRef((void**) (&(*Result).key), copyString(((NimStringDesc*) &TMP1667)));
	unsureAsgnRef((void**) (&(*Result).val), copyString(((NimStringDesc*) &TMP1667)));
}
...
N_NIMCALL(void, handleshortoption_199401)(Optparser199205* p) {
	...
	}
	{
		...
		i = parseword_199218((*p).cmd, i, (&(*p).val), TMP1668);
	}
	LA5: ;
	{
...
{	i = (*p).pos;
	...
	case 45:
	{
		i += ((NI) 1);
		{
			...
			(*p).kind = ((NU8) 2);
			i += ((NI) 1);
			i = parseword_199218((*p).cmd, i, (&(*p).key), TMP1666);
			{
				while (1) {
			...
			}
			{
				...
				(*p).pos = parseword_199218((*p).cmd, i, (&(*p).val), TMP1668);
			}
			goto LA15;
			LA17: ;
			{
	...
	{
		(*p).kind = ((NU8) 1);
		(*p).pos = parseword_199218((*p).cmd, i, (&(*p).key), TMP1668);
	}
	break;
	}
	}BeforeRet: ;
...
N_NIMCALL(NimStringDesc*, npocmdLineRest)(Optparser199205* p) {
	...
	result = nsuStrip(LOC1, NIM_TRUE, NIM_TRUE, TMP1673);
	return result;
}
...
static TNimNode* TMP1660[6];
static TNimNode* TMP1661[4];
NI TMP1663;
static char* NIM_CONST TMP1662[4] = {
"cmdEnd", 
"cmdArgument", 
"cmdLongOption", 
"cmdShortOption"};
...
NTI199203.base = 0;
NTI199203.flags = 3;
for (TMP1663 = 0; TMP1663 < 4; TMP1663++) {
TMP788[TMP1663+5].kind = 1;
TMP788[TMP1663+5].offset = TMP1663;
TMP788[TMP1663+5].name = TMP1662[TMP1663];
TMP1661[TMP1663] = &TMP788[TMP1663+5];
}
TMP788[9].len = 4; TMP788[9].kind = 2; TMP788[9].sons = &TMP1661[0];
...
TMP788[11].name = "val";
TMP788[0].len = 6; TMP788[0].kind = 2; TMP788[0].sons = &TMP1660[0];
