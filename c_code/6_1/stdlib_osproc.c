...
N_NIMCALL(void, createstream_144642)(streamobj136035** stream, int* handle, NU8 filemode);
...
static NIM_CONST TY196407 TMP1800 = {
...
static NIM_CONST TY188635 TMP4214 = {NIM_NIL,NIM_NIL};
...
N_NIMCALL(processobj141402*, nospstartProcess)(NimStringDesc* command, NimStringDesc* workingdir, NimStringDesc** args, NI argsLen0, stringtableobj132012* env, NU8 options) {
	...
	if (TMP2875.status == 0) {
		...
		TMP2877.status = _setjmp(TMP2877.context);
		if (TMP2877.status == 0) {
...
{	result = 0;
	{
		...
		createstream_144642(&(*p).Instream, (&(*p).Inhandle), ((NU8) 1));
	}
	LA3: ;
	result = (*p).Instream;
	goto BeforeRet;
...
{	result = 0;
	{
		...
		createstream_144642(&(*p).Outstream, (&(*p).Outhandle), ((NU8) 0));
	}
	LA3: ;
	result = (*p).Outstream;
	goto BeforeRet;
