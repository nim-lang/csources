...
static NIM_CONST TY196407 TMP1574 = {
...
N_NIMCALL(void, handleshortoption_192309)(optparser192011* p) {
	...
	}
	{
		...
		i = parseword_192204((*p).Cmd, i, (&(*p).Val), TMP1574);
	}
	LA5: ;
	{
...
{	i = (*p).Pos;
	...
	case 45:
	{
		i += 1;
		{
			...
			(*p).Kind = ((NU8) 2);
			i += 1;
			i = parseword_192204((*p).Cmd, i, (&(*p).Key), TMP1575);
			{
				while (1) {
			...
			}
			{
				...
				(*p).Pos = parseword_192204((*p).Cmd, i, (&(*p).Val), TMP1574);
			}
			goto LA15;
			LA17: ;
			{
	...
	{
		(*p).Kind = ((NU8) 1);
		(*p).Pos = parseword_192204((*p).Cmd, i, (&(*p).Key), TMP1574);
	}
	break;
	}
	}BeforeRet: ;
