...
STRING_LITERAL(TMP4634, "renderPlainSymbolName() with ", 29);
STRING_LITERAL(TMP4641, "ref.", 4);
STRING_LITERAL(TMP4642, "ptr.", 4);
STRING_LITERAL(TMP4643, "proc(", 5);
STRING_LITERAL(TMP4644, "tuple[", 6);
STRING_LITERAL(TMP4645, "renderParamTypes(found,n) with ", 31);
...
N_NIMCALL(NimStringDesc*, renderplainsymbolname_393013)(tnode224819* n) {
	...
	result = 0;
	result = copyString(((NimStringDesc*) &TMP4632));
	switch ((*n).Kind) {
	case ((NU8) 31):
	{
		{
	...
	{
		{
			if (!!(eqStrings((*(*n).kindU.S5.Ident).S, ((NimStringDesc*) &TMP4633)))) goto LA15;
...
appendString(LOC24, ((NimStringDesc*) &TMP4634));
...
appendString(LOC6, ((NimStringDesc*) &TMP4641));
...
appendString(LOC10, ((NimStringDesc*) &TMP4642));
...
appendString(LOC31, ((NimStringDesc*) &TMP4645));
