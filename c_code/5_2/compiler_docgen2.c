...
N_NIMCALL(Tnode214790*, close_515009)(Tpasscontext261002* p, Tnode214790* n) {
	...
	usewarning = !((((*(*g).module).flags &(1U<<((NU)(((NU8) 12))&31U)))!=0));
	{
		...
		writeoutput_361697((*g).doc, LOC7, ((NimStringDesc*) &TMP5163), usewarning);
		pushSafePoint(&TMP5164);
		TMP5164.status = _setjmp(TMP5164.context);
		if (TMP5164.status == 0) {
		...
		if (TMP5164.status != 0) reraiseException();
	}
	LA5: ;
	return result;
}
...
TMP1185[0].len = 2; TMP1185[0].kind = 2; TMP1185[0].sons = &TMP5161[0];
NTI515004.node = &TMP1185[0];
NTI515006.size = sizeof(Tgen515004*);
NTI515006.kind = 22;
NTI515006.base = (&NTI515004);
NTI515006.marker = TMP5162;
}
