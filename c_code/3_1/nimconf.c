/* Generated by Nimrod Compiler v0.9.3 */
/*   (c) 2014 Andreas Rumpf */
/* The generated code is subject to the original license. */
#define NIM_INTBITS 32
#include "nimbase.h"

#include <string.h>

#include <stdio.h>
typedef struct tlexer178176 tlexer178176;
typedef struct ttoken178174 ttoken178174;
typedef struct tident166019 tident166019;
typedef struct NimStringDesc NimStringDesc;
typedef struct TGenericSeq TGenericSeq;
typedef struct tbaselexer176018 tbaselexer176018;
typedef struct TNimObject TNimObject;
typedef struct TNimType TNimType;
typedef struct TNimNode TNimNode;
typedef struct tllstream174204 tllstream174204;
typedef struct tidobj166013 tidobj166013;
typedef struct TY182131 TY182131;
typedef struct tcell41490 tcell41490;
typedef struct tcellseq41506 tcellseq41506;
typedef struct tgcheap43416 tgcheap43416;
typedef struct tcellset41502 tcellset41502;
typedef struct tpagedesc41498 tpagedesc41498;
typedef struct tmemregion23810 tmemregion23810;
typedef struct tsmallchunk23038 tsmallchunk23038;
typedef struct tllchunk23804 tllchunk23804;
typedef struct tbigchunk23040 tbigchunk23040;
typedef struct tintset23015 tintset23015;
typedef struct ttrunk23011 ttrunk23011;
typedef struct tavlnode23808 tavlnode23808;
typedef struct tgcstat43414 tgcstat43414;
typedef struct tlineinfo162527 tlineinfo162527;
typedef struct tbasechunk23036 tbasechunk23036;
typedef struct tfreecell23028 tfreecell23028;
struct  TGenericSeq  {
NI len;
NI reserved;
};
struct  NimStringDesc  {
  TGenericSeq Sup;
NIM_CHAR data[SEQ_DECL_SIZE];
};
struct  ttoken178174  {
NU8 Toktype;
NI Indent;
tident166019* Ident;
NI64 Inumber;
NF Fnumber;
NU8 Base;
NI8 Strongspacea;
NI8 Strongspaceb;
NimStringDesc* Literal;
NI Line;
NI Col;
};
typedef N_NIMCALL_PTR(void, TY2289) (void* p, NI op);
struct  TNimType  {
NI size;
NU8 kind;
NU8 flags;
TNimType* base;
TNimNode* node;
void* finalizer;
TY2289 marker;
};
struct  TNimObject  {
TNimType* m_type;
};
struct  tbaselexer176018  {
  TNimObject Sup;
NI Bufpos;
NCSTRING Buf;
NI Buflen;
tllstream174204* Stream;
NI Linenumber;
NI Sentinel;
NI Linestart;
};
struct  tlexer178176  {
  tbaselexer176018 Sup;
NI32 Fileidx;
NI Indentahead;
NI Currlineindent;
NIM_BOOL Strongspaces;
};
struct  tidobj166013  {
  TNimObject Sup;
NI Id;
};
struct  tident166019  {
  tidobj166013 Sup;
NimStringDesc* S;
tident166019* Next;
NI H;
};
struct  TNimNode  {
NU8 kind;
NI offset;
TNimType* typ;
NCSTRING name;
NI len;
TNimNode** sons;
};
struct  tcell41490  {
NI Refcount;
TNimType* Typ;
};
struct  tcellseq41506  {
NI Len;
NI Cap;
tcell41490** D;
};
struct  tcellset41502  {
NI Counter;
NI Max;
tpagedesc41498* Head;
tpagedesc41498** Data;
};
typedef tsmallchunk23038* TY23822[512];
typedef ttrunk23011* ttrunkbuckets23013[256];
struct  tintset23015  {
ttrunkbuckets23013 Data;
};
struct  tmemregion23810  {
NI Minlargeobj;
NI Maxlargeobj;
TY23822 Freesmallchunks;
tllchunk23804* Llmem;
NI Currmem;
NI Maxmem;
NI Freemem;
NI Lastsize;
tbigchunk23040* Freechunkslist;
tintset23015 Chunkstarts;
tavlnode23808* Root;
tavlnode23808* Deleted;
tavlnode23808* Last;
tavlnode23808* Freeavlnodes;
};
struct  tgcstat43414  {
NI Stackscans;
NI Cyclecollections;
NI Maxthreshold;
NI Maxstacksize;
NI Maxstackcells;
NI Cycletablesize;
NI64 Maxpause;
};
struct  tgcheap43416  {
void* Stackbottom;
NI Cyclethreshold;
tcellseq41506 Zct;
tcellseq41506 Decstack;
tcellset41502 Cycleroots;
tcellseq41506 Tempstack;
NI Recgclock;
tmemregion23810 Region;
tgcstat43414 Stat;
};
struct  tlineinfo162527  {
NI16 Line;
NI16 Col;
NI32 Fileindex;
};
typedef NimStringDesc* TY128092[3];
struct  tllstream174204  {
  TNimObject Sup;
NU8 Kind;
FILE* F;
NimStringDesc* S;
NI Rd;
NI Wr;
NI Lineoffset;
};
typedef NI TY23018[16];
struct  tpagedesc41498  {
tpagedesc41498* Next;
NI Key;
TY23018 Bits;
};
struct  tbasechunk23036  {
NI Prevsize;
NI Size;
NIM_BOOL Used;
};
struct  tsmallchunk23038  {
  tbasechunk23036 Sup;
tsmallchunk23038* Next;
tsmallchunk23038* Prev;
tfreecell23028* Freelist;
NI Free;
NI Acc;
NF Data;
};
struct  tllchunk23804  {
NI Size;
NI Acc;
tllchunk23804* Next;
};
struct  tbigchunk23040  {
  tbasechunk23036 Sup;
tbigchunk23040* Next;
tbigchunk23040* Prev;
NI Align;
NF Data;
};
struct  ttrunk23011  {
ttrunk23011* Next;
NI Key;
TY23018 Bits;
};
typedef tavlnode23808* TY23814[2];
struct  tavlnode23808  {
TY23814 Link;
NI Key;
NI Upperbound;
NI Level;
};
struct  tfreecell23028  {
tfreecell23028* Next;
NI Zerofield;
};
struct TY182131 {
  TGenericSeq Sup;
  NIM_BOOL data[SEQ_DECL_SIZE];
};
N_NIMCALL(void, ppgettok_182008)(tlexer178176* l, ttoken178174* tok);
N_NIMCALL(void, rawgettok_178211)(tlexer178176* l, ttoken178174* tok);
static N_INLINE(void, nimFrame)(TFrame* s);
static N_INLINE(void, popFrame)(void);
N_NIMCALL(NIM_BOOL, parseatom_182038)(tlexer178176* l, ttoken178174* tok);
N_NIMCALL(void, lexmessage_178246)(tlexer178176* l, NU16 msg, NimStringDesc* arg);
N_NIMCALL(NIM_BOOL, isdefined_167022)(tident166019* symbol);
N_NIMCALL(NIM_BOOL, parseandexpr_182074)(tlexer178176* l, ttoken178174* tok);
N_NIMCALL(NIM_BOOL, parseexpr_182031)(tlexer178176* l, ttoken178174* tok);
N_NIMCALL(NIM_BOOL, evalppif_182113)(tlexer178176* l, ttoken178174* tok);
N_NIMCALL(void, TMP1899)(void* p, NI op);
static N_INLINE(void, nimGCunrefNoCycle)(void* p);
static N_INLINE(tcell41490*, usrtocell_44843)(void* usr);
static N_INLINE(void, rtladdzct_46402)(tcell41490* c);
N_NOINLINE(void, addzct_44818)(tcellseq41506* s, tcell41490* c);
N_NIMCALL(void*, newSeqRC1)(TNimType* typ, NI len);
N_NIMCALL(void, doend_182155)(tlexer178176* l, ttoken178174* tok);
N_NIMCALL(TGenericSeq*, setLengthSeq)(TGenericSeq* seq, NI elemsize, NI newlen);
N_NIMCALL(void, doelse_182193)(tlexer178176* l, ttoken178174* tok);
N_NOINLINE(void, raiseIndexError)(void);
N_NIMCALL(void, doelif_182211)(tlexer178176* l, ttoken178174* tok);
N_NIMCALL(void, jumptodirective_182185)(tlexer178176* l, ttoken178174* tok, NU8 dest);
static N_INLINE(NIM_BOOL, eqStrings)(NimStringDesc* a, NimStringDesc* b);
N_NIMCALL(NU8, whichkeyword_170360)(tident166019* id);
static N_INLINE(NI, addInt)(NI a, NI b);
N_NOINLINE(void, raiseOverflow)(void);
static N_INLINE(NI, subInt)(NI a, NI b);
N_NIMCALL(void, parsedirective_182309)(tlexer178176* l, ttoken178174* tok);
N_NIMCALL(void, msgwriteln_163937)(NimStringDesc* s);
N_NIMCALL(NimStringDesc*, toktostr_178234)(ttoken178174* tok);
N_NIMCALL(NimStringDesc*, nsuNormalize)(NimStringDesc* s);
N_NIMCALL(void, putenv_112643)(NimStringDesc* key, NimStringDesc* val);
static N_INLINE(void, appendString)(NimStringDesc* dest, NimStringDesc* src);
N_NIMCALL(NimStringDesc*, getenv_112626)(NimStringDesc* key);
N_NIMCALL(NimStringDesc*, rawNewString)(NI space);
N_NIMCALL(void, conftok_182348)(tlexer178176* l, ttoken178174* tok);
N_NIMCALL(void, checksymbol_182374)(tlexer178176* l, ttoken178174* tok);
N_NIMCALL(void, parseassignment_182407)(tlexer178176* l, ttoken178174* tok);
N_NIMCALL(tident166019*, getident_166456)(NimStringDesc* identifier);
static N_INLINE(tlineinfo162527, getlineinfo_178218)(tlexer178176* l, ttoken178174* tok);
N_NIMCALL(tlineinfo162527, newlineinfo_163069)(NI32 fileinfoidx, NI line, NI col);
N_NIMCALL(NimStringDesc*, copyString)(NimStringDesc* src);
N_NIMCALL(NimStringDesc*, addChar)(NimStringDesc* s, NIM_CHAR c);
N_NIMCALL(NimStringDesc*, resizeString)(NimStringDesc* dest, NI addlen);
N_NIMCALL(void, processswitch_172016)(NimStringDesc* switch_172018, NimStringDesc* arg, NU8 pass, tlineinfo162527 info);
N_NIMCALL(void, readconfigfile_182479)(NimStringDesc* filename);
N_NIMCALL(tllstream174204*, llstreamopen_174227)(NimStringDesc* filename, NU8 mode);
N_NIMCALL(void, inittoken_178417)(ttoken178174* l);
N_NIMCALL(void, openlexer_178238)(tlexer178176* lex, NimStringDesc* filename, tllstream174204* inputstream);
N_NIMCALL(void, closelexer_178225)(tlexer178176* lex);
N_NIMCALL(void, rawmessage_164386)(NU16 msg, NimStringDesc* arg);
N_NIMCALL(NimStringDesc*, getuserconfigpath_182536)(NimStringDesc* filename);
N_NIMCALL(NimStringDesc*, nosjoinPath)(NimStringDesc* head, NimStringDesc* tail);
N_NIMCALL(NimStringDesc*, nosgetConfigDir)(void);
N_NIMCALL(NimStringDesc*, getsystemconfigpath_182544)(NimStringDesc* filename);
N_NIMCALL(NimStringDesc*, getprefixdir_136330)(void);
N_NIMCALL(NimStringDesc*, nosjoinPathOpenArray)(NimStringDesc** parts, NI partsLen0);
N_NIMCALL(NIM_BOOL, nosexistsFile)(NimStringDesc* filename);
N_NIMCALL(void, loadconfigs_182607)(NimStringDesc* cfg);
N_NIMCALL(NimStringDesc*, copyStringRC1)(NimStringDesc* src);
static N_INLINE(void, asgnRefNoCycle)(void** dest, void* src);
N_NIMCALL(NimStringDesc*, nosgetCurrentDir)(void);
N_NIMCALL(NimStringDesc*, HEX2F_109677)(NimStringDesc* head, NimStringDesc* tail);
N_NIMCALL(NIM_BOOL, nosisRootDir)(NimStringDesc* path);
N_NIMCALL(NimStringDesc*, nosparentDir)(NimStringDesc* path);
N_NIMCALL(NimStringDesc*, copyStrLast)(NimStringDesc* s, NI start_71024, NI last);
N_NIMCALL(NimStringDesc*, copyStrLast)(NimStringDesc* s, NI first, NI last);
N_NIMCALL(NimStringDesc*, noschangeFileExt)(NimStringDesc* filename, NimStringDesc* ext);
STRING_LITERAL(TMP1897, "\')\'", 3);
STRING_LITERAL(TMP1898, "\':\'", 3);
STRING_LITERAL(TMP1900, "@if", 3);
STRING_LITERAL(TMP1901, "@", 1);
STRING_LITERAL(TMP1902, "@end", 4);
STRING_LITERAL(TMP1904, "putenv", 6);
STRING_LITERAL(TMP1905, "prependenv", 10);
STRING_LITERAL(TMP1906, "appendenv", 9);
STRING_LITERAL(TMP1907, "-", 1);
STRING_LITERAL(TMP1908, "--", 2);
STRING_LITERAL(TMP1909, "", 0);
STRING_LITERAL(TMP1910, "\']\'", 3);
STRING_LITERAL(TMP1911, "&", 1);
STRING_LITERAL(TMP1915, "config", 6);
STRING_LITERAL(TMP1916, "etc", 3);
STRING_LITERAL(TMP1917, "/etc/", 5);
STRING_LITERAL(TMP1918, "/usr", 4);
STRING_LITERAL(TMP1919, "/usr/lib/nimrod", 15);
STRING_LITERAL(TMP1920, "/usr/local", 10);
STRING_LITERAL(TMP1921, "/usr/local/lib/nimrod", 21);
STRING_LITERAL(TMP1922, "lib", 3);
STRING_LITERAL(TMP1925, "nimrod.cfg", 10);
extern TFrame* frameptr_13238;
TY182131* condstack_182152;
extern TNimType NTI134; /* bool */
TNimType NTI182131; /* seq[bool] */
extern tgcheap43416 gch_43444;
extern TNimType NTI178176; /* TLexer */
extern NI gverbosity_136120;
extern NimStringDesc* libpath_136205;
extern NU32 gglobaloptions_136111;
extern NimStringDesc* gprojectpath_136207;
extern NimStringDesc* gprojectname_136206;
extern NimStringDesc* gprojectfull_136208;

static N_INLINE(void, nimFrame)(TFrame* s) {
	(*s).prev = frameptr_13238;
	frameptr_13238 = s;
}

static N_INLINE(void, popFrame)(void) {
	frameptr_13238 = (*frameptr_13238).prev;
}

N_NIMCALL(void, ppgettok_182008)(tlexer178176* l, ttoken178174* tok) {
	nimfr("ppGetTok", "nimconf.nim")
	nimln(21, "nimconf.nim");
	rawgettok_178211(l, tok);
	nimln(22, "nimconf.nim");
	while (1) {
		nimln(22, "nimconf.nim");
		if (!((*tok).Toktype == ((NU8) 111))) goto LA1;
		nimln(22, "nimconf.nim");
		rawgettok_178211(l, tok);
	} LA1: ;
	popFrame();
}

N_NIMCALL(NIM_BOOL, parseatom_182038)(tlexer178176* l, ttoken178174* tok) {
	NIM_BOOL result;
	nimfr("parseAtom", "nimconf.nim")
	result = 0;
	nimln(26, "nimconf.nim");
	{
		nimln(26, "nimconf.nim");
		if (!((*tok).Toktype == ((NU8) 91))) goto LA3;
		nimln(27, "nimconf.nim");
		ppgettok_182008(l, tok);
		nimln(28, "nimconf.nim");
		result = parseexpr_182031(l, tok);
		nimln(29, "nimconf.nim");
		{
			nimln(29, "nimconf.nim");
			if (!((*tok).Toktype == ((NU8) 92))) goto LA7;
			nimln(29, "nimconf.nim");
			ppgettok_182008(l, tok);
		}
		goto LA5;
		LA7: ;
		{
			nimln(30, "nimconf.nim");
			lexmessage_178246(&(*l), ((NU16) 23), ((NimStringDesc*) &TMP1897));
		}
		LA5: ;
	}
	goto LA1;
	LA3: ;
	{
		NIM_BOOL LOC13;
		nimln(31, "nimconf.nim");
		if (!((*(*tok).Ident).Sup.Id == 43)) goto LA11;
		nimln(32, "nimconf.nim");
		ppgettok_182008(l, tok);
		nimln(33, "nimconf.nim");
		nimln(33, "nimconf.nim");
		nimln(33, "nimconf.nim");
		LOC13 = 0;
		LOC13 = parseatom_182038(l, tok);
		result = !(LOC13);
	}
	goto LA1;
	LA11: ;
	{
		nimln(35, "nimconf.nim");
		result = isdefined_167022((*tok).Ident);
		nimln(36, "nimconf.nim");
		ppgettok_182008(l, tok);
	}
	LA1: ;
	popFrame();
	return result;
}

N_NIMCALL(NIM_BOOL, parseandexpr_182074)(tlexer178176* l, ttoken178174* tok) {
	NIM_BOOL result;
	nimfr("parseAndExpr", "nimconf.nim")
	result = 0;
	nimln(39, "nimconf.nim");
	result = parseatom_182038(l, tok);
	nimln(40, "nimconf.nim");
	while (1) {
		NIM_BOOL b;
		NIM_BOOL LOC2;
		nimln(40, "nimconf.nim");
		if (!((*(*tok).Ident).Sup.Id == 2)) goto LA1;
		nimln(41, "nimconf.nim");
		ppgettok_182008(l, tok);
		nimln(42, "nimconf.nim");
		b = parseatom_182038(l, tok);
		nimln(43, "nimconf.nim");
		nimln(43, "nimconf.nim");
		LOC2 = 0;
		LOC2 = result;
		if (!(LOC2)) goto LA3;
		LOC2 = b;
		LA3: ;
		result = LOC2;
	} LA1: ;
	popFrame();
	return result;
}

N_NIMCALL(NIM_BOOL, parseexpr_182031)(tlexer178176* l, ttoken178174* tok) {
	NIM_BOOL result;
	nimfr("parseExpr", "nimconf.nim")
	result = 0;
	nimln(46, "nimconf.nim");
	result = parseandexpr_182074(l, tok);
	nimln(47, "nimconf.nim");
	while (1) {
		NIM_BOOL b;
		NIM_BOOL LOC2;
		nimln(47, "nimconf.nim");
		if (!((*(*tok).Ident).Sup.Id == 47)) goto LA1;
		nimln(48, "nimconf.nim");
		ppgettok_182008(l, tok);
		nimln(49, "nimconf.nim");
		b = parseandexpr_182074(l, tok);
		nimln(50, "nimconf.nim");
		nimln(50, "nimconf.nim");
		LOC2 = 0;
		LOC2 = result;
		if (LOC2) goto LA3;
		LOC2 = b;
		LA3: ;
		result = LOC2;
	} LA1: ;
	popFrame();
	return result;
}

N_NIMCALL(NIM_BOOL, evalppif_182113)(tlexer178176* l, ttoken178174* tok) {
	NIM_BOOL result;
	nimfr("evalppIf", "nimconf.nim")
	result = 0;
	nimln(53, "nimconf.nim");
	ppgettok_182008(l, tok);
	nimln(54, "nimconf.nim");
	result = parseexpr_182031(l, tok);
	nimln(55, "nimconf.nim");
	{
		nimln(55, "nimconf.nim");
		if (!((*tok).Toktype == ((NU8) 105))) goto LA3;
		nimln(55, "nimconf.nim");
		ppgettok_182008(l, tok);
	}
	goto LA1;
	LA3: ;
	{
		nimln(56, "nimconf.nim");
		lexmessage_178246(&(*l), ((NU16) 23), ((NimStringDesc*) &TMP1898));
	}
	LA1: ;
	popFrame();
	return result;
}
N_NIMCALL(void, TMP1899)(void* p, NI op) {
	TY182131* a;
	NI LOC1;
	a = (TY182131*)p;
	LOC1 = 0;
	for (LOC1 = 0; LOC1 < a->Sup.len; LOC1++) {
	}
}

static N_INLINE(tcell41490*, usrtocell_44843)(void* usr) {
	tcell41490* result;
	nimfr("usrToCell", "gc.nim")
	result = 0;
	nimln(118, "gc.nim");
	nimln(118, "gc.nim");
	nimln(118, "gc.nim");
	result = ((tcell41490*) ((NI)((NU32)(((NI) (usr))) - (NU32)(((NI)sizeof(tcell41490))))));
	popFrame();
	return result;
}

static N_INLINE(void, rtladdzct_46402)(tcell41490* c) {
	nimfr("rtlAddZCT", "gc.nim")
	nimln(199, "gc.nim");
	addzct_44818(&gch_43444.Zct, c);
	popFrame();
}

static N_INLINE(void, nimGCunrefNoCycle)(void* p) {
	tcell41490* c;
	nimfr("nimGCunrefNoCycle", "gc.nim")
	nimln(233, "gc.nim");
	c = usrtocell_44843(p);
	nimln(235, "gc.nim");
	{
		nimln(167, "gc.nim");
		(*c).Refcount -= 8;
		nimln(168, "gc.nim");
		if (!((NU32)((*c).Refcount) < (NU32)(8))) goto LA3;
		nimln(236, "gc.nim");
		rtladdzct_46402(c);
	}
	LA3: ;
	popFrame();
}

N_NIMCALL(void, doend_182155)(tlexer178176* l, ttoken178174* tok) {
	nimfr("doEnd", "nimconf.nim")
	nimln(61, "nimconf.nim");
	{
		nimln(61, "nimconf.nim");
		nimln(61, "nimconf.nim");
		if (!((condstack_182152->Sup.len-1) < 0)) goto LA3;
		nimln(61, "nimconf.nim");
		lexmessage_178246(&(*l), ((NU16) 23), ((NimStringDesc*) &TMP1900));
	}
	LA3: ;
	nimln(62, "nimconf.nim");
	ppgettok_182008(l, tok);
	nimln(63, "nimconf.nim");
	nimln(63, "nimconf.nim");
	condstack_182152 = (TY182131*) setLengthSeq(&(condstack_182152)->Sup, sizeof(NIM_BOOL), (condstack_182152->Sup.len-1));
	popFrame();
}

N_NIMCALL(void, doelse_182193)(tlexer178176* l, ttoken178174* tok) {
	nimfr("doElse", "nimconf.nim")
	nimln(71, "nimconf.nim");
	{
		nimln(71, "nimconf.nim");
		nimln(71, "nimconf.nim");
		if (!((condstack_182152->Sup.len-1) < 0)) goto LA3;
		nimln(71, "nimconf.nim");
		lexmessage_178246(&(*l), ((NU16) 23), ((NimStringDesc*) &TMP1900));
	}
	LA3: ;
	nimln(72, "nimconf.nim");
	ppgettok_182008(l, tok);
	nimln(73, "nimconf.nim");
	{
		nimln(73, "nimconf.nim");
		if (!((*tok).Toktype == ((NU8) 105))) goto LA7;
		nimln(73, "nimconf.nim");
		ppgettok_182008(l, tok);
	}
	LA7: ;
	nimln(74, "nimconf.nim");
	{
		nimln(74, "nimconf.nim");
		if ((NU)((condstack_182152->Sup.len-1)) >= (NU)(condstack_182152->Sup.len)) raiseIndexError();
		if (!condstack_182152->data[(condstack_182152->Sup.len-1)]) goto LA11;
		nimln(74, "nimconf.nim");
		jumptodirective_182185(l, tok, ((NU8) 0));
	}
	LA11: ;
	popFrame();
}

N_NIMCALL(void, doelif_182211)(tlexer178176* l, ttoken178174* tok) {
	NIM_BOOL res;
	nimfr("doElif", "nimconf.nim")
	nimln(77, "nimconf.nim");
	{
		nimln(77, "nimconf.nim");
		nimln(77, "nimconf.nim");
		if (!((condstack_182152->Sup.len-1) < 0)) goto LA3;
		nimln(77, "nimconf.nim");
		lexmessage_178246(&(*l), ((NU16) 23), ((NimStringDesc*) &TMP1900));
	}
	LA3: ;
	nimln(78, "nimconf.nim");
	res = evalppif_182113(l, tok);
	nimln(79, "nimconf.nim");
	{
		NIM_BOOL LOC7;
		nimln(79, "nimconf.nim");
		LOC7 = 0;
		nimln(79, "nimconf.nim");
		if ((NU)((condstack_182152->Sup.len-1)) >= (NU)(condstack_182152->Sup.len)) raiseIndexError();
		LOC7 = condstack_182152->data[(condstack_182152->Sup.len-1)];
		if (LOC7) goto LA8;
		nimln(79, "nimconf.nim");
		LOC7 = !(res);
		LA8: ;
		if (!LOC7) goto LA9;
		nimln(79, "nimconf.nim");
		jumptodirective_182185(l, tok, ((NU8) 1));
	}
	goto LA5;
	LA9: ;
	{
		nimln(80, "nimconf.nim");
		nimln(80, "nimconf.nim");
		if ((NU)((condstack_182152->Sup.len-1)) >= (NU)(condstack_182152->Sup.len)) raiseIndexError();
		condstack_182152->data[(condstack_182152->Sup.len-1)] = NIM_TRUE;
	}
	LA5: ;
	popFrame();
}

static N_INLINE(NIM_BOOL, eqStrings)(NimStringDesc* a, NimStringDesc* b) {
	NIM_BOOL result;
	NIM_BOOL LOC11;
	int LOC13;
	result = 0;
	{
		if (!(a == b)) goto LA3;
		result = NIM_TRUE;
		goto BeforeRet;
	}
	LA3: ;
	{
		NIM_BOOL LOC7;
		LOC7 = 0;
		LOC7 = (a == NIM_NIL);
		if (LOC7) goto LA8;
		LOC7 = (b == NIM_NIL);
		LA8: ;
		if (!LOC7) goto LA9;
		result = NIM_FALSE;
		goto BeforeRet;
	}
	LA9: ;
	LOC11 = 0;
	LOC11 = ((*a).Sup.len == (*b).Sup.len);
	if (!(LOC11)) goto LA12;
	LOC13 = 0;
	LOC13 = memcmp(((NCSTRING) ((*a).data)), ((NCSTRING) ((*b).data)), (NI32)((*a).Sup.len * 1));
	LOC11 = (LOC13 == ((NI32) 0));
	LA12: ;
	result = LOC11;
	goto BeforeRet;
	BeforeRet: ;
	return result;
}

static N_INLINE(NI, addInt)(NI a, NI b) {
	NI result;
	result = 0;
	result = (NI)((NU32)(a) + (NU32)(b));
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (0 <= (NI)(result ^ a));
		if (LOC3) goto LA4;
		LOC3 = (0 <= (NI)(result ^ b));
		LA4: ;
		if (!LOC3) goto LA5;
		goto BeforeRet;
	}
	LA5: ;
	raiseOverflow();
	BeforeRet: ;
	return result;
}

static N_INLINE(NI, subInt)(NI a, NI b) {
	NI result;
	result = 0;
	result = (NI)((NU32)(a) - (NU32)(b));
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (0 <= (NI)(result ^ a));
		if (LOC3) goto LA4;
		LOC3 = (0 <= (NI)(result ^ (NI)((NU32) ~(b))));
		LA4: ;
		if (!LOC3) goto LA5;
		goto BeforeRet;
	}
	LA5: ;
	raiseOverflow();
	BeforeRet: ;
	return result;
}

N_NIMCALL(void, jumptodirective_182185)(tlexer178176* l, ttoken178174* tok, NU8 dest) {
	NI nestedifs;
	nimfr("jumpToDirective", "nimconf.nim")
	nimln(83, "nimconf.nim");
	nestedifs = 0;
	nimln(84, "nimconf.nim");
	while (1) {
		nimln(85, "nimconf.nim");
		{
			NIM_BOOL LOC4;
			NU8 LOC8;
			nimln(85, "nimconf.nim");
			LOC4 = 0;
			nimln(85, "nimconf.nim");
			nimln(85, "nimconf.nim");
			LOC4 = !(((*tok).Ident == NIM_NIL));
			if (!(LOC4)) goto LA5;
			nimln(85, "nimconf.nim");
			LOC4 = eqStrings((*(*tok).Ident).S, ((NimStringDesc*) &TMP1901));
			LA5: ;
			if (!LOC4) goto LA6;
			nimln(86, "nimconf.nim");
			ppgettok_182008(l, tok);
			nimln(87, "nimconf.nim");
			nimln(87, "nimconf.nim");
			LOC8 = 0;
			LOC8 = whichkeyword_170360((*tok).Ident);
			switch (LOC8) {
			case ((NU8) 28):
			{
				nimln(89, "nimconf.nim");
				nestedifs = addInt(nestedifs, 1);
			}
			break;
			case ((NU8) 19):
			{
				nimln(91, "nimconf.nim");
				{
					NIM_BOOL LOC13;
					nimln(91, "nimconf.nim");
					LOC13 = 0;
					nimln(91, "nimconf.nim");
					LOC13 = (dest == ((NU8) 1));
					if (!(LOC13)) goto LA14;
					nimln(91, "nimconf.nim");
					LOC13 = (nestedifs == 0);
					LA14: ;
					if (!LOC13) goto LA15;
					nimln(92, "nimconf.nim");
					doelse_182193(l, tok);
					nimln(93, "nimconf.nim");
					goto LA1;
				}
				LA15: ;
			}
			break;
			case ((NU8) 18):
			{
				nimln(95, "nimconf.nim");
				{
					NIM_BOOL LOC20;
					nimln(95, "nimconf.nim");
					LOC20 = 0;
					nimln(95, "nimconf.nim");
					LOC20 = (dest == ((NU8) 1));
					if (!(LOC20)) goto LA21;
					nimln(95, "nimconf.nim");
					LOC20 = (nestedifs == 0);
					LA21: ;
					if (!LOC20) goto LA22;
					nimln(96, "nimconf.nim");
					doelif_182211(l, tok);
					nimln(97, "nimconf.nim");
					goto LA1;
				}
				LA22: ;
			}
			break;
			case ((NU8) 20):
			{
				nimln(99, "nimconf.nim");
				{
					nimln(99, "nimconf.nim");
					if (!(nestedifs == 0)) goto LA27;
					nimln(100, "nimconf.nim");
					doend_182155(l, tok);
					nimln(101, "nimconf.nim");
					goto LA1;
				}
				LA27: ;
				nimln(102, "nimconf.nim");
				{
					nimln(102, "nimconf.nim");
					if (!(0 < nestedifs)) goto LA31;
					nimln(102, "nimconf.nim");
					nestedifs = subInt(nestedifs, 1);
				}
				LA31: ;
			}
			break;
			default:
			{
			}
			break;
			}
			nimln(105, "nimconf.nim");
			ppgettok_182008(l, tok);
		}
		goto LA2;
		LA6: ;
		{
			nimln(106, "nimconf.nim");
			if (!((*tok).Toktype == ((NU8) 1))) goto LA35;
			nimln(107, "nimconf.nim");
			lexmessage_178246(&(*l), ((NU16) 23), ((NimStringDesc*) &TMP1902));
		}
		goto LA2;
		LA35: ;
		{
			nimln(109, "nimconf.nim");
			ppgettok_182008(l, tok);
		}
		LA2: ;
	} LA1: ;
	popFrame();
}

static N_INLINE(void, appendString)(NimStringDesc* dest, NimStringDesc* src) {
	memcpy(((NCSTRING) (&(*dest).data[((*dest).Sup.len)- 0])), ((NCSTRING) ((*src).data)), (NI32)((*src).Sup.len + 1));
	(*dest).Sup.len += (*src).Sup.len;
}

N_NIMCALL(void, parsedirective_182309)(tlexer178176* l, ttoken178174* tok) {
	NU8 LOC1;
	nimfr("parseDirective", "nimconf.nim")
	nimln(112, "nimconf.nim");
	ppgettok_182008(l, tok);
	nimln(113, "nimconf.nim");
	nimln(113, "nimconf.nim");
	LOC1 = 0;
	LOC1 = whichkeyword_170360((*tok).Ident);
	switch (LOC1) {
	case ((NU8) 28):
	{
		NI TMP1903;
		NIM_BOOL res;
		nimln(115, "nimconf.nim");
		nimln(115, "nimconf.nim");
		nimln(115, "nimconf.nim");
		TMP1903 = addInt(condstack_182152->Sup.len, 1);
		condstack_182152 = (TY182131*) setLengthSeq(&(condstack_182152)->Sup, sizeof(NIM_BOOL), (NI32)(TMP1903));
		nimln(116, "nimconf.nim");
		res = evalppif_182113(l, tok);
		nimln(117, "nimconf.nim");
		nimln(117, "nimconf.nim");
		if ((NU)((condstack_182152->Sup.len-1)) >= (NU)(condstack_182152->Sup.len)) raiseIndexError();
		condstack_182152->data[(condstack_182152->Sup.len-1)] = res;
		nimln(118, "nimconf.nim");
		{
			nimln(118, "nimconf.nim");
			if (!!(res)) goto LA5;
			nimln(118, "nimconf.nim");
			jumptodirective_182185(l, tok, ((NU8) 1));
		}
		LA5: ;
	}
	break;
	case ((NU8) 18):
	{
		nimln(119, "nimconf.nim");
		doelif_182211(l, tok);
	}
	break;
	case ((NU8) 19):
	{
		nimln(120, "nimconf.nim");
		doelse_182193(l, tok);
	}
	break;
	case ((NU8) 20):
	{
		nimln(121, "nimconf.nim");
		doend_182155(l, tok);
	}
	break;
	case ((NU8) 175):
	{
		NimStringDesc* LOC11;
		nimln(123, "nimconf.nim");
		ppgettok_182008(l, tok);
		nimln(124, "nimconf.nim");
		nimln(124, "nimconf.nim");
		LOC11 = 0;
		LOC11 = toktostr_178234(&(*tok));
		msgwriteln_163937(LOC11);
		nimln(125, "nimconf.nim");
		ppgettok_182008(l, tok);
	}
	break;
	default:
	{
		NimStringDesc* LOC13;
		nimln(127, "nimconf.nim");
		nimln(127, "nimconf.nim");
		LOC13 = 0;
		LOC13 = nsuNormalize((*(*tok).Ident).S);
		if (eqStrings(LOC13, ((NimStringDesc*) &TMP1904))) goto LA14;
		if (eqStrings(LOC13, ((NimStringDesc*) &TMP1905))) goto LA15;
		if (eqStrings(LOC13, ((NimStringDesc*) &TMP1906))) goto LA16;
		goto LA17;
		LA14: ;
		{
			NimStringDesc* key;
			NimStringDesc* LOC20;
			nimln(129, "nimconf.nim");
			ppgettok_182008(l, tok);
			nimln(130, "nimconf.nim");
			key = toktostr_178234(&(*tok));
			nimln(131, "nimconf.nim");
			ppgettok_182008(l, tok);
			nimln(132, "nimconf.nim");
			nimln(132, "nimconf.nim");
			LOC20 = 0;
			LOC20 = toktostr_178234(&(*tok));
			putenv_112643(key, LOC20);
			nimln(133, "nimconf.nim");
			ppgettok_182008(l, tok);
		}
		goto LA18;
		LA15: ;
		{
			NimStringDesc* key;
			NimStringDesc* LOC22;
			NimStringDesc* LOC23;
			NimStringDesc* LOC24;
			nimln(135, "nimconf.nim");
			ppgettok_182008(l, tok);
			nimln(136, "nimconf.nim");
			key = toktostr_178234(&(*tok));
			nimln(137, "nimconf.nim");
			ppgettok_182008(l, tok);
			nimln(138, "nimconf.nim");
			nimln(138, "nimconf.nim");
			LOC22 = 0;
			nimln(138, "nimconf.nim");
			LOC23 = 0;
			LOC23 = toktostr_178234(&(*tok));
			nimln(138, "nimconf.nim");
			LOC24 = 0;
			LOC24 = getenv_112626(key);
			LOC22 = rawNewString(LOC23->Sup.len + LOC24->Sup.len + 0);
appendString(LOC22, LOC23);
appendString(LOC22, LOC24);
			putenv_112643(key, LOC22);
			nimln(139, "nimconf.nim");
			ppgettok_182008(l, tok);
		}
		goto LA18;
		LA16: ;
		{
			NimStringDesc* key;
			NimStringDesc* LOC26;
			NimStringDesc* LOC27;
			NimStringDesc* LOC28;
			nimln(141, "nimconf.nim");
			ppgettok_182008(l, tok);
			nimln(142, "nimconf.nim");
			key = toktostr_178234(&(*tok));
			nimln(143, "nimconf.nim");
			ppgettok_182008(l, tok);
			nimln(144, "nimconf.nim");
			nimln(144, "nimconf.nim");
			LOC26 = 0;
			nimln(144, "nimconf.nim");
			LOC27 = 0;
			LOC27 = getenv_112626(key);
			nimln(144, "nimconf.nim");
			LOC28 = 0;
			LOC28 = toktostr_178234(&(*tok));
			LOC26 = rawNewString(LOC27->Sup.len + LOC28->Sup.len + 0);
appendString(LOC26, LOC27);
appendString(LOC26, LOC28);
			putenv_112643(key, LOC26);
			nimln(145, "nimconf.nim");
			ppgettok_182008(l, tok);
		}
		goto LA18;
		LA17: ;
		{
			NimStringDesc* LOC30;
			nimln(146, "nimconf.nim");
			nimln(146, "nimconf.nim");
			LOC30 = 0;
			LOC30 = toktostr_178234(&(*tok));
			lexmessage_178246(&(*l), ((NU16) 30), LOC30);
		}
		LA18: ;
	}
	break;
	}
	popFrame();
}

N_NIMCALL(void, conftok_182348)(tlexer178176* l, ttoken178174* tok) {
	nimfr("confTok", "nimconf.nim")
	nimln(149, "nimconf.nim");
	ppgettok_182008(l, tok);
	nimln(150, "nimconf.nim");
	while (1) {
		NIM_BOOL LOC2;
		nimln(150, "nimconf.nim");
		LOC2 = 0;
		nimln(150, "nimconf.nim");
		nimln(150, "nimconf.nim");
		LOC2 = !(((*tok).Ident == NIM_NIL));
		if (!(LOC2)) goto LA3;
		nimln(150, "nimconf.nim");
		LOC2 = eqStrings((*(*tok).Ident).S, ((NimStringDesc*) &TMP1901));
		LA3: ;
		if (!LOC2) goto LA1;
		nimln(151, "nimconf.nim");
		parsedirective_182309(l, tok);
	} LA1: ;
	popFrame();
}

N_NIMCALL(void, checksymbol_182374)(tlexer178176* l, ttoken178174* tok) {
	nimfr("checkSymbol", "nimconf.nim")
	nimln(154, "nimconf.nim");
	{
		NimStringDesc* LOC5;
		nimln(154, "nimconf.nim");
		nimln(154, "nimconf.nim");
		if (!!(((*tok).Toktype >= ((NU8) 2) && (*tok).Toktype <= ((NU8) 70) || (*tok).Toktype >= ((NU8) 85) && (*tok).Toktype <= ((NU8) 87)))) goto LA3;
		nimln(155, "nimconf.nim");
		nimln(155, "nimconf.nim");
		LOC5 = 0;
		LOC5 = toktostr_178234(tok);
		lexmessage_178246(l, ((NU16) 19), LOC5);
	}
	LA3: ;
	popFrame();
}

static N_INLINE(tlineinfo162527, getlineinfo_178218)(tlexer178176* l, ttoken178174* tok) {
	tlineinfo162527 result;
	nimfr("getLineInfo", "lexer.nim")
	memset((void*)&result, 0, sizeof(result));
	nimln(137, "lexer.nim");
	result = newlineinfo_163069((*l).Fileidx, (*tok).Line, (*tok).Col);
	popFrame();
	return result;
}

N_NIMCALL(void, parseassignment_182407)(tlexer178176* l, ttoken178174* tok) {
	tlineinfo162527 info;
	NimStringDesc* s;
	NimStringDesc* val;
	nimfr("parseAssignment", "nimconf.nim")
	nimln(158, "nimconf.nim");
	{
		NIM_BOOL LOC3;
		tident166019* LOC4;
		tident166019* LOC6;
		nimln(158, "nimconf.nim");
		LOC3 = 0;
		nimln(158, "nimconf.nim");
		nimln(158, "nimconf.nim");
		LOC4 = 0;
		LOC4 = getident_166456(((NimStringDesc*) &TMP1907));
		LOC3 = ((*(*tok).Ident).Sup.Id == (*LOC4).Sup.Id);
		if (LOC3) goto LA5;
		nimln(158, "nimconf.nim");
		nimln(158, "nimconf.nim");
		LOC6 = 0;
		LOC6 = getident_166456(((NimStringDesc*) &TMP1908));
		LOC3 = ((*(*tok).Ident).Sup.Id == (*LOC6).Sup.Id);
		LA5: ;
		if (!LOC3) goto LA7;
		nimln(159, "nimconf.nim");
		conftok_182348(l, tok);
	}
	LA7: ;
	nimln(160, "nimconf.nim");
	info = getlineinfo_178218(&(*l), &(*tok));
	nimln(161, "nimconf.nim");
	checksymbol_182374(&(*l), &(*tok));
	nimln(162, "nimconf.nim");
	s = toktostr_178234(&(*tok));
	nimln(163, "nimconf.nim");
	conftok_182348(l, tok);
	nimln(164, "nimconf.nim");
	val = copyString(((NimStringDesc*) &TMP1909));
	nimln(165, "nimconf.nim");
	while (1) {
		NimStringDesc* LOC10;
		nimln(165, "nimconf.nim");
		if (!((*tok).Toktype == ((NU8) 108))) goto LA9;
		nimln(166, "nimconf.nim");
		s = addChar(s, 46);
		nimln(167, "nimconf.nim");
		conftok_182348(l, tok);
		nimln(168, "nimconf.nim");
		checksymbol_182374(&(*l), &(*tok));
		nimln(169, "nimconf.nim");
		nimln(169, "nimconf.nim");
		LOC10 = 0;
		LOC10 = toktostr_178234(&(*tok));
		s = resizeString(s, LOC10->Sup.len + 0);
appendString(s, LOC10);
		nimln(170, "nimconf.nim");
		conftok_182348(l, tok);
	} LA9: ;
	nimln(171, "nimconf.nim");
	{
		NimStringDesc* LOC15;
		nimln(171, "nimconf.nim");
		if (!((*tok).Toktype == ((NU8) 93))) goto LA13;
		nimln(174, "nimconf.nim");
		conftok_182348(l, tok);
		nimln(175, "nimconf.nim");
		checksymbol_182374(&(*l), &(*tok));
		nimln(176, "nimconf.nim");
		nimln(176, "nimconf.nim");
		LOC15 = 0;
		LOC15 = toktostr_178234(&(*tok));
		val = resizeString(val, LOC15->Sup.len + 0);
appendString(val, LOC15);
		nimln(177, "nimconf.nim");
		conftok_182348(l, tok);
		nimln(178, "nimconf.nim");
		{
			nimln(178, "nimconf.nim");
			if (!((*tok).Toktype == ((NU8) 94))) goto LA18;
			nimln(178, "nimconf.nim");
			conftok_182348(l, tok);
		}
		goto LA16;
		LA18: ;
		{
			nimln(179, "nimconf.nim");
			lexmessage_178246(&(*l), ((NU16) 23), ((NimStringDesc*) &TMP1910));
		}
		LA16: ;
		nimln(180, "nimconf.nim");
		val = addChar(val, 93);
	}
	LA13: ;
	nimln(181, "nimconf.nim");
	{
		NimStringDesc* LOC29;
		nimln(181, "nimconf.nim");
		if (!((*tok).Toktype == ((NU8) 105) || (*tok).Toktype == ((NU8) 107))) goto LA23;
		nimln(182, "nimconf.nim");
		{
			nimln(182, "nimconf.nim");
			nimln(182, "nimconf.nim");
			if (!(0 < val->Sup.len)) goto LA27;
			nimln(182, "nimconf.nim");
			val = addChar(val, 58);
		}
		LA27: ;
		nimln(183, "nimconf.nim");
		conftok_182348(l, tok);
		nimln(184, "nimconf.nim");
		checksymbol_182374(&(*l), &(*tok));
		nimln(185, "nimconf.nim");
		nimln(185, "nimconf.nim");
		LOC29 = 0;
		LOC29 = toktostr_178234(&(*tok));
		val = resizeString(val, LOC29->Sup.len + 0);
appendString(val, LOC29);
		nimln(186, "nimconf.nim");
		conftok_182348(l, tok);
		nimln(187, "nimconf.nim");
		while (1) {
			NIM_BOOL LOC31;
			tident166019* LOC33;
			NimStringDesc* LOC34;
			nimln(187, "nimconf.nim");
			LOC31 = 0;
			nimln(187, "nimconf.nim");
			nimln(187, "nimconf.nim");
			LOC31 = !(((*tok).Ident == NIM_NIL));
			if (!(LOC31)) goto LA32;
			nimln(187, "nimconf.nim");
			nimln(187, "nimconf.nim");
			LOC33 = 0;
			LOC33 = getident_166456(((NimStringDesc*) &TMP1911));
			LOC31 = ((*(*tok).Ident).Sup.Id == (*LOC33).Sup.Id);
			LA32: ;
			if (!LOC31) goto LA30;
			nimln(188, "nimconf.nim");
			conftok_182348(l, tok);
			nimln(189, "nimconf.nim");
			checksymbol_182374(&(*l), &(*tok));
			nimln(190, "nimconf.nim");
			nimln(190, "nimconf.nim");
			LOC34 = 0;
			LOC34 = toktostr_178234(&(*tok));
			val = resizeString(val, LOC34->Sup.len + 0);
appendString(val, LOC34);
			nimln(191, "nimconf.nim");
			conftok_182348(l, tok);
		} LA30: ;
	}
	LA23: ;
	nimln(192, "nimconf.nim");
	processswitch_172016(s, val, ((NU8) 2), info);
	popFrame();
}

N_NIMCALL(void, readconfigfile_182479)(NimStringDesc* filename) {
	tlexer178176 l;
	ttoken178174 tok;
	tllstream174204* stream;
	nimfr("readConfigFile", "nimconf.nim")
	memset((void*)&l, 0, sizeof(l));
	l.Sup.Sup.m_type = (&NTI178176);
	memset((void*)&tok, 0, sizeof(tok));
	stream = 0;
	nimln(199, "nimconf.nim");
	stream = llstreamopen_174227(filename, ((NU8) 0));
	nimln(200, "nimconf.nim");
	{
		nimln(200, "nimconf.nim");
		nimln(200, "nimconf.nim");
		if (!!((stream == NIM_NIL))) goto LA3;
		nimln(201, "nimconf.nim");
		inittoken_178417(&tok);
		nimln(202, "nimconf.nim");
		openlexer_178238(&l, filename, stream);
		nimln(203, "nimconf.nim");
		tok.Toktype = ((NU8) 1);
		nimln(204, "nimconf.nim");
		conftok_182348(&l, &tok);
		nimln(205, "nimconf.nim");
		while (1) {
			nimln(205, "nimconf.nim");
			nimln(205, "nimconf.nim");
			if (!!((tok.Toktype == ((NU8) 1)))) goto LA5;
			nimln(205, "nimconf.nim");
			parseassignment_182407(&l, &tok);
		} LA5: ;
		nimln(206, "nimconf.nim");
		{
			nimln(206, "nimconf.nim");
			nimln(206, "nimconf.nim");
			if (!(0 < condstack_182152->Sup.len)) goto LA8;
			nimln(206, "nimconf.nim");
			lexmessage_178246(&l, ((NU16) 23), ((NimStringDesc*) &TMP1902));
		}
		LA8: ;
		nimln(207, "nimconf.nim");
		closelexer_178225(&l);
		nimln(208, "nimconf.nim");
		{
			nimln(208, "nimconf.nim");
			if (!(1 <= gverbosity_136120)) goto LA12;
			nimln(208, "nimconf.nim");
			rawmessage_164386(((NU16) 268), filename);
		}
		LA12: ;
	}
	LA3: ;
	popFrame();
}

N_NIMCALL(NimStringDesc*, getuserconfigpath_182536)(NimStringDesc* filename) {
	NimStringDesc* result;
	NimStringDesc* LOC1;
	nimfr("getUserConfigPath", "nimconf.nim")
	result = 0;
	nimln(211, "nimconf.nim");
	nimln(211, "nimconf.nim");
	LOC1 = 0;
	LOC1 = nosgetConfigDir();
	result = nosjoinPath(LOC1, filename);
	popFrame();
	return result;
}

N_NIMCALL(NimStringDesc*, getsystemconfigpath_182544)(NimStringDesc* filename) {
	NimStringDesc* result;
	NimStringDesc* p;
	TY128092 LOC1;
	nimfr("getSystemConfigPath", "nimconf.nim")
	result = 0;
	nimln(216, "nimconf.nim");
	p = getprefixdir_136330();
	nimln(217, "nimconf.nim");
	memset((void*)LOC1, 0, sizeof(LOC1));
	LOC1[0] = copyString(p);
	LOC1[1] = copyString(((NimStringDesc*) &TMP1915));
	LOC1[2] = copyString(filename);
	result = nosjoinPathOpenArray(LOC1, 3);
	nimln(219, "nimconf.nim");
	{
		NIM_BOOL LOC4;
		TY128092 LOC7;
		nimln(219, "nimconf.nim");
		nimln(219, "nimconf.nim");
		LOC4 = 0;
		LOC4 = nosexistsFile(result);
		if (!!(LOC4)) goto LA5;
		nimln(219, "nimconf.nim");
		memset((void*)LOC7, 0, sizeof(LOC7));
		LOC7[0] = copyString(p);
		LOC7[1] = copyString(((NimStringDesc*) &TMP1916));
		LOC7[2] = copyString(filename);
		result = nosjoinPathOpenArray(LOC7, 3);
	}
	LA5: ;
	nimln(220, "nimconf.nim");
	{
		NIM_BOOL LOC10;
		NimStringDesc* LOC13;
		nimln(220, "nimconf.nim");
		nimln(220, "nimconf.nim");
		LOC10 = 0;
		LOC10 = nosexistsFile(result);
		if (!!(LOC10)) goto LA11;
		nimln(220, "nimconf.nim");
		nimln(220, "nimconf.nim");
		LOC13 = 0;
		LOC13 = rawNewString(filename->Sup.len + 5);
appendString(LOC13, ((NimStringDesc*) &TMP1917));
appendString(LOC13, filename);
		result = LOC13;
	}
	LA11: ;
	popFrame();
	return result;
}

static N_INLINE(void, asgnRefNoCycle)(void** dest, void* src) {
	nimfr("asgnRefNoCycle", "gc.nim")
	nimln(251, "gc.nim");
	{
		tcell41490* c;
		nimln(251, "gc.nim");
		nimln(251, "gc.nim");
		if (!!((src == NIM_NIL))) goto LA3;
		nimln(252, "gc.nim");
		c = usrtocell_44843(src);
		nimln(169, "gc.nim");
		(*c).Refcount += 8;
	}
	LA3: ;
	nimln(254, "gc.nim");
	{
		tcell41490* c;
		nimln(254, "gc.nim");
		nimln(254, "gc.nim");
		if (!!(((*dest) == NIM_NIL))) goto LA7;
		nimln(255, "gc.nim");
		c = usrtocell_44843((*dest));
		nimln(256, "gc.nim");
		{
			nimln(167, "gc.nim");
			(*c).Refcount -= 8;
			nimln(168, "gc.nim");
			if (!((NU32)((*c).Refcount) < (NU32)(8))) goto LA11;
			nimln(257, "gc.nim");
			rtladdzct_46402(c);
		}
		LA11: ;
	}
	LA7: ;
	nimln(258, "gc.nim");
	(*dest) = src;
	popFrame();
}

N_NIMCALL(void, loadconfigs_182607)(NimStringDesc* cfg) {
	NimStringDesc* pd;
	nimfr("loadConfigs", "nimconf.nim")
	nimln(224, "nimconf.nim");
	{
		NimStringDesc* prefix;
		nimln(224, "nimconf.nim");
		if (!((libpath_136205) && (libpath_136205)->Sup.len == 0)) goto LA3;
		nimln(226, "nimconf.nim");
		prefix = getprefixdir_136330();
		nimln(227, "nimconf.nim");
		{
			NimStringDesc* LOC9;
			nimln(227, "nimconf.nim");
			if (!eqStrings(prefix, ((NimStringDesc*) &TMP1918))) goto LA7;
			nimln(227, "nimconf.nim");
			LOC9 = 0;
			LOC9 = libpath_136205; libpath_136205 = copyStringRC1(((NimStringDesc*) &TMP1919));
			if (LOC9) nimGCunrefNoCycle(LOC9);
		}
		goto LA5;
		LA7: ;
		{
			NimStringDesc* LOC13;
			nimln(228, "nimconf.nim");
			if (!eqStrings(prefix, ((NimStringDesc*) &TMP1920))) goto LA11;
			nimln(228, "nimconf.nim");
			LOC13 = 0;
			LOC13 = libpath_136205; libpath_136205 = copyStringRC1(((NimStringDesc*) &TMP1921));
			if (LOC13) nimGCunrefNoCycle(LOC13);
		}
		goto LA5;
		LA11: ;
		{
			nimln(229, "nimconf.nim");
			asgnRefNoCycle((void**) &libpath_136205, nosjoinPath(prefix, ((NimStringDesc*) &TMP1922)));
		}
		LA5: ;
	}
	LA3: ;
	nimln(231, "nimconf.nim");
	{
		NimStringDesc* LOC19;
		nimln(231, "nimconf.nim");
		nimln(231, "nimconf.nim");
		if (!!(((gglobaloptions_136111 &(1<<((((NU8) 16))&31)))!=0))) goto LA17;
		nimln(232, "nimconf.nim");
		nimln(232, "nimconf.nim");
		LOC19 = 0;
		LOC19 = getsystemconfigpath_182544(cfg);
		readconfigfile_182479(LOC19);
	}
	LA17: ;
	nimln(234, "nimconf.nim");
	{
		NimStringDesc* LOC24;
		nimln(234, "nimconf.nim");
		nimln(234, "nimconf.nim");
		if (!!(((gglobaloptions_136111 &(1<<((((NU8) 18))&31)))!=0))) goto LA22;
		nimln(235, "nimconf.nim");
		nimln(235, "nimconf.nim");
		LOC24 = 0;
		LOC24 = getuserconfigpath_182536(cfg);
		readconfigfile_182479(LOC24);
	}
	LA22: ;
	nimln(237, "nimconf.nim");
	nimln(237, "nimconf.nim");
	{
		nimln(237, "nimconf.nim");
		nimln(237, "nimconf.nim");
		if (!(0 < gprojectpath_136207->Sup.len)) goto LA27;
		pd = copyString(gprojectpath_136207);
	}
	goto LA25;
	LA27: ;
	{
		nimln(237, "nimconf.nim");
		pd = nosgetCurrentDir();
	}
	LA25: ;
	nimln(238, "nimconf.nim");
	{
		NimStringDesc* dir_182654;
		nimln(238, "nimconf.nim");
		nimln(238, "nimconf.nim");
		if (!!(((gglobaloptions_136111 &(1<<((((NU8) 19))&31)))!=0))) goto LA32;
		dir_182654 = 0;
		nimln(621, "os.nim");
		{
			NimStringDesc* current_182669;
			if (!NIM_FALSE) goto LA36;
			nimln(622, "os.nim");
			current_182669 = copyString(pd);
			nimln(623, "os.nim");
			{
				NimStringDesc* LOC42;
				if (!NIM_FALSE) goto LA40;
				nimln(239, "nimconf.nim");
				dir_182654 = pd;
				nimln(240, "nimconf.nim");
				nimln(240, "nimconf.nim");
				LOC42 = 0;
				LOC42 = HEX2F_109677(dir_182654, cfg);
				readconfigfile_182479(LOC42);
			}
			LA40: ;
			nimln(624, "os.nim");
			while (1) {
				NimStringDesc* LOC49;
				nimln(625, "os.nim");
				{
					NIM_BOOL LOC46;
					nimln(625, "os.nim");
					LOC46 = 0;
					LOC46 = nosisRootDir(current_182669);
					if (!LOC46) goto LA47;
					nimln(625, "os.nim");
					goto LA43;
				}
				LA47: ;
				nimln(626, "os.nim");
				current_182669 = nosparentDir(current_182669);
				nimln(622, "os.nim");
				dir_182654 = current_182669;
				nimln(240, "nimconf.nim");
				nimln(240, "nimconf.nim");
				LOC49 = 0;
				LOC49 = HEX2F_109677(dir_182654, cfg);
				readconfigfile_182479(LOC49);
			} LA43: ;
		}
		goto LA34;
		LA36: ;
		{
			NI i_182671;
			NI HEX3Atmp_182673;
			NI TMP1923;
			NI res_182675;
			i_182671 = 0;
			HEX3Atmp_182673 = 0;
			nimln(629, "os.nim");
			nimln(629, "os.nim");
			nimln(629, "os.nim");
			TMP1923 = subInt(pd->Sup.len, 2);
			HEX3Atmp_182673 = (NI32)(TMP1923);
			nimln(1450, "system.nim");
			res_182675 = 0;
			nimln(1451, "system.nim");
			while (1) {
				nimln(1451, "system.nim");
				if (!(res_182675 <= HEX3Atmp_182673)) goto LA51;
				nimln(1450, "system.nim");
				i_182671 = res_182675;
				nimln(631, "os.nim");
				{
					NIM_BOOL LOC54;
					NIM_BOOL LOC56;
					NI TMP1924;
					NimStringDesc* LOC60;
					nimln(631, "os.nim");
					LOC54 = 0;
					nimln(631, "os.nim");
					if ((NU)(i_182671) > (NU)(pd->Sup.len)) raiseIndexError();
					LOC54 = (((NU8)(pd->data[i_182671])) == ((NU8)(47)) || ((NU8)(pd->data[i_182671])) == ((NU8)(47)));
					if (!(LOC54)) goto LA55;
					nimln(632, "os.nim");
					LOC56 = 0;
					nimln(632, "os.nim");
					LOC56 = (i_182671 == 0);
					if (LOC56) goto LA57;
					nimln(632, "os.nim");
					nimln(632, "os.nim");
					nimln(632, "os.nim");
					TMP1924 = subInt(i_182671, 1);
					if ((NU)((NI32)(TMP1924)) > (NU)(pd->Sup.len)) raiseIndexError();
					LOC56 = !((((NU8)(pd->data[(NI32)(TMP1924)])) == ((NU8)(47)) || ((NU8)(pd->data[(NI32)(TMP1924)])) == ((NU8)(47))));
					LA57: ;
					LOC54 = LOC56;
					LA55: ;
					if (!LOC54) goto LA58;
					nimln(633, "os.nim");
					nimln(633, "os.nim");
					dir_182654 = copyStrLast(pd, 0, i_182671);
					nimln(240, "nimconf.nim");
					nimln(240, "nimconf.nim");
					LOC60 = 0;
					LOC60 = HEX2F_109677(dir_182654, cfg);
					readconfigfile_182479(LOC60);
				}
				LA58: ;
				nimln(1453, "system.nim");
				res_182675 = addInt(res_182675, 1);
			} LA51: ;
			nimln(635, "os.nim");
			{
				NimStringDesc* LOC65;
				if (!NIM_FALSE) goto LA63;
				nimln(239, "nimconf.nim");
				dir_182654 = pd;
				nimln(240, "nimconf.nim");
				nimln(240, "nimconf.nim");
				LOC65 = 0;
				LOC65 = HEX2F_109677(dir_182654, cfg);
				readconfigfile_182479(LOC65);
			}
			LA63: ;
		}
		LA34: ;
	}
	LA32: ;
	nimln(242, "nimconf.nim");
	{
		NimStringDesc* LOC70;
		nimln(242, "nimconf.nim");
		nimln(242, "nimconf.nim");
		if (!!(((gglobaloptions_136111 &(1<<((((NU8) 17))&31)))!=0))) goto LA68;
		nimln(243, "nimconf.nim");
		nimln(243, "nimconf.nim");
		LOC70 = 0;
		LOC70 = HEX2F_109677(pd, cfg);
		readconfigfile_182479(LOC70);
		nimln(245, "nimconf.nim");
		{
			NimStringDesc* LOC75;
			nimln(245, "nimconf.nim");
			nimln(245, "nimconf.nim");
			nimln(245, "nimconf.nim");
			if (!!((gprojectname_136206->Sup.len == 0))) goto LA73;
			nimln(247, "nimconf.nim");
			nimln(247, "nimconf.nim");
			LOC75 = 0;
			LOC75 = noschangeFileExt(gprojectfull_136208, ((NimStringDesc*) &TMP1925));
			readconfigfile_182479(LOC75);
		}
		LA73: ;
	}
	LA68: ;
	popFrame();
}
N_NOINLINE(void, nimconfInit)(void) {
	nimfr("nimconf", "nimconf.nim")
	nimln(58, "nimconf.nim");
	if (condstack_182152) nimGCunrefNoCycle(condstack_182152);
	condstack_182152 = (TY182131*) newSeqRC1((&NTI182131), 0);
	popFrame();
}

N_NOINLINE(void, nimconfDatInit)(void) {
NTI182131.size = sizeof(TY182131*);
NTI182131.kind = 24;
NTI182131.base = (&NTI134);
NTI182131.flags = 2;
NTI182131.marker = TMP1899;
}

