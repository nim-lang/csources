...
static NIM_CONST TY178407 TMP1539 = {
...
N_NIMCALL(void, handleshortoption_175291)(Optparser175011* p) {
	...
	}
	{
		...
		i = parseword_175204((*p).cmd, i, (&(*p).val), TMP1539);
	}
	LA5: ;
	{
...
{	i = (*p).pos;
	...
	case 45:
	{
		i += ((NI) 1);
		{
			...
			(*p).kind = ((NU8) 2);
			i += ((NI) 1);
			i = parseword_175204((*p).cmd, i, (&(*p).key), TMP1540);
			{
				while (1) {
			...
			}
			{
				...
				(*p).pos = parseword_175204((*p).cmd, i, (&(*p).val), TMP1539);
			}
			goto LA15;
			LA17: ;
			{
	...
	{
		(*p).kind = ((NU8) 1);
		(*p).pos = parseword_175204((*p).cmd, i, (&(*p).key), TMP1539);
	}
	break;
	}
	}BeforeRet: ;
