...
STRING_LITERAL(TMP2794, "newFileSize == -1 or not mode == fmRead ", 40);
...
N_NIMCALL(tmemfile205204, open_205210)(NimStringDesc* filename, NU8 mode, NI mappedsize, NI offset, NI newfilesize) {
	...
	nimln(45, "memfiles.nim");
	{
		...
		if (!!(LOC3)) goto LA5;
		nimln(45, "memfiles.nim");
		hiddenraiseassert_71620(((NimStringDesc*) &TMP2794));
	}
	LA5: ;
...
static TNimNode* TMP2875[3];
static TNimNode TMP2792[4];
...
TMP2875[0] = &TMP2792[1];
TMP2792[1].kind = 1;
TMP2792[1].offset = offsetof(tmemfile205204, Mem);
TMP2792[1].typ = (&NTI147);
TMP2792[1].name = "mem";
TMP2875[1] = &TMP2792[2];
TMP2792[2].kind = 1;
TMP2792[2].offset = offsetof(tmemfile205204, Size);
TMP2792[2].typ = (&NTI105);
TMP2792[2].name = "size";
TMP2875[2] = &TMP2792[3];
TMP2792[3].kind = 1;
TMP2792[3].offset = offsetof(tmemfile205204, Handle);
TMP2792[3].typ = (&NTI3701);
TMP2792[3].name = "handle";
TMP2792[0].len = 3; TMP2792[0].kind = 2; TMP2792[0].sons = &TMP2875[0];
NTI205204.node = &TMP2792[0];
}
